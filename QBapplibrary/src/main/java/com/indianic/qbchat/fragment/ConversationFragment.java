...
import com.indianic.qbchat.adapter.OpponentsFromCallAdapter;
import com.indianic.qbchat.utils.CameraUtils;
import com.indianic.qbchat.utils.QbConstant;
import com.indianic.qbchat.view.RTCGLVideoView;
 ...
 * QuickBlox team
 */
public class ConversationFragment extends Fragment implements Serializable, QBRTCClientVideoTracksCallbacks, QBRTCSessionConnectionCallbacks, QbActivity.QBRTCSessionUserCallback, OpponentsFromCallAdapter.OnAdapterEventListener {
    ...
    private Map<String, String> stringUserMap;

    public static ConversationFragment newInstance(ArrayList<QBUser> opponents, String callerName, QBRTCTypes.QBConferenceType qbConferenceType, Map<String, String> userInfo, QbActivity.StartConversetionReason reason, String sesionnId) {

        ConversationFragment fragment = new ConversationFragment();
        Bundle bundle = new Bundle();
        bundle.putInt(QbConstant.CONFERENCE_TYPE, qbConferenceType.getValue());
        bundle.putString(CALLER_NAME, callerName);
        bundle.putSerializable(QbConstant.OPPONENTS, (Serializable) opponents);
        bundle.putSerializable(QbConstant.USERINFO, (Serializable) userInfo);
//        if (userInfo != null) {
...
//        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        // ((QbActivity) getActivity()).initActionBarWithTimer();

        if (getArguments() != null) {
            stringUserMap = (Map<String, String>) getArguments().get(QbConstant.USERINFO);
            opponents = ((QbActivity) getActivity()).getUserList();
            qbConferenceType = getArguments().getInt(QbConstant.CONFERENCE_TYPE);
            startReason = getArguments().getInt(QbActivity.START_CONVERSATION_REASON);
            sessionID = getArguments().getString(QbActivity.SESSION_ID);
            callerName = getArguments().getString(QbActivity.CALLER_NAME);
    ...
    private void initSessionListener() {
        ((QbActivity) getActivity()).addVideoTrackCallbacksListener(this);
    }
    ...
    public void onStart() {
        ...
        super.onStart();
        QBRTCSession session = ((QbActivity) getActivity()).getCurrentSession();
        if (!isMessageProcessed) {
            if (startReason == QbActivity.StartConversetionReason.INCOME_CALL_FOR_ACCEPTION.ordinal()) {
                session.acceptCall(session.getUserInfo());
            } else {
                session.startCall(stringUserMap);
            }
            isMessageProcessed = true;
        }
        ((QbActivity) getActivity()).addTCClientConnectionCallback(this);
        ((QbActivity) getActivity()).addRTCSessionUserCallback(this);
    }
    ...
    private void initViews(View view) {
        ...
        recyclerView.addItemDecoration(new DividerItemDecoration(getActivity(), R.dimen.qb_4dp));
...
//                DataHolder.getUserIndexByFullName(callerName)) + 1));
    ...
    private void setGrid(int columnsCount, int rowsCount) {
        ...
        float itemMargin = getResources().getDimension(R.dimen.qb_4dp);
    ...
    public void onStop() {
        ...
        ((QbActivity) getActivity()).removeRTCClientConnectionCallback(this);
        ((QbActivity) getActivity()).removeRTCSessionUserCallback(this);
    }
    ...
    private void initButtonsListener() {
        ...
        dynamicToggleVideoCall.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (((QbActivity) getActivity()).getCurrentSession() != null) {
                    Log.d(TAG, "Dynamic switched!");
                    ((QbActivity) getActivity()).getCurrentSession().switchAudioOutput();
                }
            }
        });
            ...
            public void onClick(View v) {
                if (((QbActivity) getActivity()).getCurrentSession() != null) {
                    if (isAudioEnabled) {
                        ...
                        ((QbActivity) getActivity()).getCurrentSession().setAudioEnabled(false);
                        isAudioEnabled = false;
                    } else {
                        Log.d(TAG, "Mic is on!");
                        ((QbActivity) getActivity()).getCurrentSession().setAudioEnabled(true);
                        isAudioEnabled = true;
                    }
                }
            }
    ...
    private View.OnClickListener getCameraSwitchListener() {
        return new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                QBRTCSession currentSession = ((QbActivity) getActivity()).getCurrentSession();
